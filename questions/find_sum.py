...
def find_two_target_sum_ordered(integers, target_sum):
    """
    Find 2 integers from the given list that sum up to target_sum.
    Params:
    integers (int[]): list of random integers in ascending order.
    target_sum (int): the target sum.
    """
    print(f'Attempting to find for sum {target_sum}.')
...
if __name__ == '__main__':
    ...
    execution_time = timeit(
        "find_two_target_sum_ordered([current_integer for current_integer in range(1, random_max)], randint(2, random_max * 2 - 2))", globals=globals(), number=1)
